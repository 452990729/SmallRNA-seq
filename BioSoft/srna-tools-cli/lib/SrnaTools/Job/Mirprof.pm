...
sub run{
  ...
  $self->_run_module(
    'run_patman',{
      ...
      remove_infiles => 1,
      same_strand => 1, # was 's'
      mismatches =>  $self->param_or_default('mismatches'), # was 'e'
  ...
  $self->_run_module(
    'mirprof',{
      ...
      ignore_org            => (defined $self->param('group_organisms')) ? 1 : 0,
      ignore_mis            => (defined $self->param('group_mismatches')) ? 1 : 0,
      collapse_match_groups => (defined $self->param('collapse_match_groups')) ? 1 : 0,
      group_family          => (defined $self->param('group_family')) ? 1 : 0,
      group_variant         => (defined $self->param('group_variant')) ? 1 : 0,
      keep_best             => (defined $self->param('keep_best')) ? 1 : 0,
    } 
  ) ;
...
sub _do_validate_user_parameters{
  ...
  if ($self->param_or_default('mismatches') && $self->param_or_default('mismatches')!~/^[0123]$/) {
...
sub _get_mirbase_file{
  ...
  my $flavour = (defined $self->param('overhangs') ) ? 'fxname' : 'fname' ; #with Xs or without
