...
if($organisms{$species} ){
   $command = "$perlExec $Bin/make_html2_gb.pl -q $outdir/miRBase.mrd -k $name1$extension1 -t $organisms{$species} -y $time $opt_d $opt_o -i $outdir/${name1}_mapped.arf $starf $opt_l $opt_m -M miRNAs_expressed_all_samples_$time.csv $opt_P $opt_W";
    ...
    $t=`$command`;
     ...
     $command="$perlExec $Bin/make_html2_gb.pl -q $outdir/miRBase.mrd -k $name1$extension1 -t $species -y $time $opt_d $opt_o -i $outdir/${name1}_mapped.arf $starf $opt_l $opt_m -M miRNAs_expressed_all_samples_$time.csv $opt_P $opt_W";
    ...
    $command = "$perlExec $Bin/make_html2_gb.pl -q $outdir/miRBase.mrd -k $name1$extension1 -y $time $opt_d $opt_o -i $outdir/${name1}_mapped.arf $starf $opt_l $opt_m -M miRNAs_expressed_all_samples_$time.csv $opt_P $opt_W";
...
## map reads against precursors
    ...
    $err=`$bowtie/bowtie -p $threads -f -v $mismatches -a --best --strata miRNA_precursor $name2.converted --al ${name2}.map.fas --un ${name2}.unmap.fas ${name2}_mapped.bwt`;
	`ln -sf ${name2}_mapped.bwt ${name2}_mapped.bwt.ka`;
    $err=`$bowtie/bowtie -p $threads -f -v $mismatches -k 1 miRNA_precursor ${name2}.map.fas ${name2}_mapped.bwt.k1`;
...
sub CreateOutputMRD{
	...
	$reads=`$mirdeep2/convert_bowtie_output.pl ${name2}_mapped.bwt > ${name2}_mapped.arf`;
