...
my %mat_sample;
open(TAG,$cvs_file);
open(CVS,">$outdir/miRNAs_expressed_all_samples_$outdir.csv");
while(<TAG>){
	chomp;
	my @ary=split(/\t/,$_);
	if(!/^#/){
		if($ary[3]>0){
			...
			if($ary[1]>0){
				...
				if($ary[0] !~ /\*$/){
					`$perlExec $srnatoolscli/srna-tools.pl --tool hp_tool --longSeq $outdir/hairpin.fa_tmp --shortSeqs $outdir/mature.fa_tmp --out $outdir/image`;
