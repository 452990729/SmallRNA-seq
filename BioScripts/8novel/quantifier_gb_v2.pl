...
use Getopt::Std;
use FindBin '$Bin'; 
#my $Bin="/PUBLIC/source/RNA/smallRNA/version3/8n_miRNA/../bin";
use Config::Tiny;
...
my $usage="usage:
  \t$perlExec quantifier.pl [options] -p precursor.fa -m mature.fa -r reads.fa -s star.fa -t species -y [timestamp] -d [pdfs] -o [sort] -k [stringent] -c config.txt -g [number of mismatches in reads vs precursor mappings]
...
[optional parameters]
  ...
  \t-T           number of alignment threads to launch during bowtie mapping
...
ConvertFastaFile($options{'r'},'novel',"","");
...
if($organisms{$species} ){
   $command = "$perlExec $Bin/make_html2_gb.pl -q $outdir/miRBase.mrd -k $name1$extension1 -t $organisms{$species} -y $time $opt_d $opt_o -i $outdir/${name1}_mapped.arf $starf $opt_l $opt_m -M $time.csv $opt_P $opt_W";
    ...
    $t=`$command`;
     ...
     $command="$perlExec $Bin/make_html2_gb.pl -q $outdir/miRBase.mrd -k $name1$extension1 -t $species -y $time $opt_d $opt_o -i $outdir/${name1}_mapped.arf $starf $opt_l $opt_m -M $time.csv $opt_P $opt_W";
    ...
    $command = "$perlExec $Bin/make_html2_gb.pl -q $outdir/miRBase.mrd -k $name1$extension1 -y $time $opt_d $opt_o -i $outdir/${name1}_mapped.arf $starf $opt_l $opt_m -M $time.csv $opt_P $opt_W";
...
## build bowtie index
    ...
    $err = `$bowtie/bowtie-build precursor.converted miRNA_precursor`;
...
## map mature sequences against precursors
	...
	if(!$options{'w'}){
		$err = `$bowtie/bowtie -p $threads -f -v 0 -a --best --strata --norc miRNA_precursor mature.converted ${name1}_mapped.bwt`;
    }

## map reads against precursors
    print STDERR "mapping read sequences against index\n";
    $err=`$bowtie/bowtie -p $threads -f -v $mismatches -a --best --strata miRNA_precursor novel.converted --al novel.map.fas --un novel.unmap.fas novel_mapped.bwt`;
	`ln -sf novel_mapped.bwt novel_mapped.bwt.ka`;
    $err=`$bowtie/bowtie -p $threads -f -v $mismatches -k 1 miRNA_precursor novel.map.fas novel_mapped.bwt.k1`;

    if($options{'s'}){
        print STDERR "mapping star sequences against index\n";
        $err = `$bowtie/bowtie -p $threads -f -v 0 -a --best --strata miRNA_precursor star.converted ${name3}_mapped.bwt`;
    }
}
...
sub ReadinReadsMappingFile{
	...
	open IN,"novel_mapped.bwt" or die "Reads mapping File novel_mapped.bwt not found \n";
	while(<IN>){
		if(/^(\S+)/){
			$mapcounts{$1}++;
		}
    ...
    while(<IN>){
        ...
        for(my $i = 1; $i <= $hash{$line[2]}{'c'}; $i++){
			if($options{'w'}){## if consider complete precursor as mature seq
				...
				$sample = $1 if($scores[0] =~ /^(\S+)_/); ## get sample id here
...
sub PrintExpressionValuesSamples{
    ...
    open OUTG,">$time.csv";
    print OUTG "#miRNA\tread_count\tprecursor\ttotal_read_count";
...
#				printf OUTG ("%s\t%.2f\t%s\t%.2f",$hash{$_}{$i}{'mature'},$hash{$_}{$i}{'score'},$_,$hash{$_}{$i}{'score'});
				printf OUTG ("%s\t%.2f\t%s\t%.2f",$hash{$_}{$i}{'mature'},$hash{$_}{$i}{'score'},$_,$hash{$_}{'r'});
				for my $sample(sort keys %hash_sample){
					next if($sample =~ /config/);
					if($hash_sample{$sample}{$_}{$i}{'score'} > 0){
#						print OUTG "\t$hash_sample{$sample}{$_}{$i}{'score'}";
...
sub CreateOutputMRD{
	...
	$mature=`$mirdeep2/convert_bowtie_output.pl ${name1}_mapped.bwt > ${name1}_mapped.arf`;
    ...
    if($options{'s'}){
        $star=`$mirdeep2/convert_bowtie_output.pl ${name3}_mapped.bwt > ${name3}_mapped.arf`;
	...
	$reads=`$mirdeep2/convert_bowtie_output.pl novel_mapped.bwt > novel_mapped.arf`;
	open IN,"<novel_mapped.arf";
	my (@rseq,@qseq);
	my $counter;
	my $col1_width = 40;
	my $spacer;
...
#old		my $rrc=$hash{$k1}{'reads'}{'tot'};
		...
		foreach(@reads_arr){
			if(/^(\S+)\s+\d+\s+\d+\s+\d+\s+(\S+)\s+\S+\s+\d+\s+(\d+)\s+(\d+)\s+\S+\s+(\S+)\s+(\d+).+$/){
				...
				$reads_hash{$rrc}{"strand"}=$5;
				$reads_hash{$rrc}{"mm"}=$6;
				if($reads_hash{$rrc}{"strand"} eq "-"){
					my $tmpSeq = reverse $reads_hash{$rrc}{"seq"};
					$tmpSeq =~ y/CGATUcgatu/gctaagctaa/;
					$reads_hash{$rrc}{"seq"} =$tmpSeq;
				}
			}
		}
		...
		foreach(@elist){                                                       ## output elist.
			...
			print OUT "$reads_hash{$_}{'id'}$spacer$bef$rseq$after\t$reads_hash{$_}{'mm'}\t$reads_hash{$_}{'strand'}\n";
		}
		print OUT "\n\n\n";
