...
my %hash2strand;                      ## read mapped strand
...
CloseHTML();
system("cp expression_analyses/expression_${time}.html expression_${time}.html");
...
sub CreateStructurePDFQuantifier{
    ...
    for(sort { $hash{$b}{"score"} <=> $hash{$a}{"score"} } keys %hash){
		...
		next if(-f "$cwd/$time\_pdfs/$filename.pdf");
...
#### main program;       
        ...
        if(not -d "$cwd/$time\_pdfs"){
            mkdir "$cwd/$time\_pdfs";
        }
        
        
        open FOLD,">$cwd/$time\_pdfs/$filename.tmp" or die "Error: cannot create tmp file$!\n";
...
#                print "$tag x \t $read r \t $sid s \n";
			...
			$hash2strand{$read}=$hash{$sid}{"reads"}{$tag}{$read}{"strand"}; ## read mapped strand with precursor sequence
...
### here the Frequency histogram is drawn
        ...
        ClosePDF($filename);
        unlink("$cwd/$time\_pdfs/${filename}_ss.ps");
        unlink("$cwd/$time\_pdfs/$filename.tmp");
        unlink("$cwd/$time\_pdfs/rna.ps");
        chdir "..";
        print STDERR "creating pdf for $filename finished\n";
    }
    unlink("$cwd/$time\_pdfs/tmp");
}
...
sub ClosePDF{
	...
	$pdf->saveas("$cwd/$time\_pdfs/$file.pdf");
}
...
#        print STDERR "$sid\t$k1\n";
    ...
    $gfx->textlabel($xposshift+110+ $lstruct_multi,$y,$trb,6,'strand' ,-color=>'black');
    $gfx->textlabel($xposshift+150+ $lstruct_multi,$y,$trb,6,'sample' ,-color=>'black');
    $y -= 10;    
    if($options{'o'}){
        for my $tag(keys %{$$hash{$sid}{"reads"}}){
            for my $k(sort { $hash2order{$a} <=> $hash2order{$b} } keys %hash2order){
                ...
                $gfx->textlabel($xposshift+110+ $lstruct_multi,$y,$trb,6,$hash2strand{$k} ,-color=>'black');
                $gfx->textlabel($xposshift+150+ $lstruct_multi,$y,$trb,6,$hash2sample{$k} ,-color=>'black');
        ...
        for my $k(sort { $hash2order{$a} <=> $hash2order{$b} } keys %hash2order){
            ...
            $gfx->textlabel($xposshift+30+ $lstruct_multi,$y,$trb,6,int($hash2c{$tag}{$hash2key{$k}}) ,-color=>'black');
            $gfx->textlabel($xposshift+70+ $lstruct_multi,$y,$trb,6,$hash2mm{$k} ,-color=>'black');
            $gfx->textlabel($xposshift+70+ $lstruct_multi,$y,$trb,6,$hash2strand{$k} ,-color=>'black');
...
sub DrawStructure{
    ...
    system("$RNAfold -d 0 < $cwd/$time\_pdfs/${filename}.tmp > $cwd/$time\_pdfs/tmp");
    ...
    open PS,"<$cwd/$time\_pdfs/rna.ps" or die "Error: cannot open $cwd/$time\_pdfs/rna.ps\t$!\n";
...
sub PrintQuantifier{
    ...
    open HTML,">expression_analyses/expression_${time}.html" or die "cannot create expression_analyses/expression_${time}.html\n";
    CreateHTML(); ##
    ...
    my %exprs;
	open IN,"<expression_analyses/miRNA_expressed.csv" or die "Error: File expression_analyses/miRNA_expressed.csv not found\n";
...
#            die "$id\t$seq\n";
    ...
    open IN,"<expression_analyses/mature2hairpin" or die "Error: File expression_analyses/mature2hairpin not found\n";
    my %hairpin2mature2;
	while(<IN>){
        chomp;
		my @line = split(/\t/);
...
#			print STDERR "yeah $1\n";
        ...
        elsif(/^(\S\S\S)(\S+)\s+(\S+)\s+(\S+)\s+(\S+)$/ and $reads){  ## do not read in known miRNAs
        $counter++;
        $hash_q{$id}{"reads"}{$1}{$counter}{"rid"} = "$1$2";
        $hash_q{$id}{"reads"}{$1}{$counter}{"seq"} = $3;
        $hash_q{$id}{"reads"}{$1}{$counter}{"mm"}  = $4;
	$hash_q{$id}{"reads"}{$1}{$counter}{"strand"}  = $5;
        }else{}
    }
    close IN;
...
#            if($id =~ /-1224/){print "$id\n";}
		...
		next if(not $hash_q{$id}{"freq_total"});
            ...
            if($options{'d'} or not $hash_q{$id}{"freq_total"}){ ## no link to pdf
                ...
                print HTML "<tr><td nowrap=\"nowrap\"><a href=\"$time\_pdfs/$id.pdf\">$id</a></td>";
            }
            print HTML <<EOF;
