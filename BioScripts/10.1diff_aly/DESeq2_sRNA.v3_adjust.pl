...
-p  <f> : pval-value threshold, default=0.05
-f  <f> : foldchange-value threshold, default=1
...
my $R_v303 = $Config->{srnaenv}->{R_v312};
...
#Res\$significant<-Res\$pval<$pv
a=Res[,3]>log2($Fc)|Res[,3]<(-log2($Fc))
b=Res\$pval<$pv
logic=a\&b
Res\$significant<-as.matrix(logic)
#------------------------------------------------------------------------------
#output files
...
sampleVolcano <- function(dat,titles){
	p=ggplot(dat, aes(x=log2FoldChange, y=-log10(pval), colour=type)) + geom_point() + coord_cartesian(xlim=c(-5, 5)) + geom_hline(yintercept=-log10($pv), linetype=2) + labs(title=titles, x=bquote(paste(log[2], "(fold change)", sep="")), y=bquote(paste(-log[10],"(pval)",sep="")))
