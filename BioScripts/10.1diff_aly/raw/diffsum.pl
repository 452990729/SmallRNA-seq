...
<Optional>
	...
	-abbr <s>  :            the species in mirbase
===============================================================================
USAGE
}
my($diff,$cutoff,$abbr,$help);
GetOptions(
        "h|?|help"=>\$help,
                "diffdir=s"=>\$diff,
                "cutoff=i"=>\$cutoff,
...
my @groups = `ls $diff/$abbr/ | grep vs | less`;
my $group;
my @diffnums;
...
foreach $group(@groups)
{
    ...
    if ($group!~ m/vs.*vs/)
    {
	my $diff_count = `wc -l $diff/$abbr/$group/*DElist.txt`;
	my $up_count = `wc -l $diff/$abbr/$group/*DElist_up.txt`;  
	my $down_count = `wc -l $diff/$abbr/$group/*DElist_down.txt`;  
