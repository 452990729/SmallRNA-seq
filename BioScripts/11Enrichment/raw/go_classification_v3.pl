...
my $out=shift;
unless(-d $out){
	!system("mkdir -p $out") or die $!;
}

my $Config = Config::Tiny -> new();
$Config = Config::Tiny ->read("$Bin/../../Pipeline/config.ini");
my $obo = $Config->{database}->{GO};      # obo file
...
barplot(bar_dat,log="y",col="black",yaxt="n",xaxt="n",space=0.75,ylim=c(0.1,120),main="Gene Function Classification (GO)")
...
mtext(side=2,line=3,"Percent of genes")
mtext(side=4,line=3,"Number of genes")
for(a in nodes){
...
open R,"|/PUBLIC/software/public/System/R-2.15.3/bin/R --vanilla --slave" or die $!;
print R $R;
close R;
