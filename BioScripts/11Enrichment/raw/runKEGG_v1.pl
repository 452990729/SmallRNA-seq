...
#my $dirSpeciesData = "/PUBLIC/database/RNA/kobas2.0-data-20120208/seq_pep_v2/";
...
if ($existGenome eq "y"){
	`$perlExec $Bin/KEGG_step2_enrich_v2.pl -id $dirDiffGeneID -out-dir $dirOutputKOBAS -species $abbrForSpecies -blast-result $dirOutputKOBAS../../../../Blast/KEGG.xml -sample-names $samplename >$dirOutputKOBAS/run.sh`;
	`sh $dirOutputKOBAS/run.sh`;
}
elsif ($existGenome eq "n"){
	!system "$perlExec $dirKOBAS_generate_run --fa $dirOutputAssemble --out-dir $dirOutputKOBAS --species $abbrForSpecies --database $database --correct-method $FDRMethod --blast-program  $blastProgram --sample-names $samplename >$dirOutputKOBAS/run.sh" or die "Something went wrong!";
