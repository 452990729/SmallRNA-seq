...
#		print "Oh, find the title\n";
		my @tmp=split /\t/;
		print "$tmp[0]\t$tmp[1]\t$tmp[2]\t$tmp[3]\t$tmp[4]\t$tmp[5]\t$tmp[6]\t$tmp[7]\tKO\tEntrez_ID\tEnsembl_ID\tKO_name\t$tmp[8]\n";
	}
	elsif (/^[\w@]/) {
		my @tmp=split /\t/;
		my @id=split/\|/,$tmp[7];
   ...
   if ( $tmp[2] !~ /ko01100|ko01110|ko01120/ ) {
       print "$tmp[0]\t$tmp[1]\t$tmp[2]\t$tmp[3]\t$tmp[4]\t$tmp[5]\t$tmp[6]\t$tmp[7]\t$ko_id\t$Entrez_id\t$Ensembl_id\t$symbol_id\t$tmp[8]\n";
     }
	}
	else
	{
