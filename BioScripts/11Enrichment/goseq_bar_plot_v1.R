...
setwd(args[2])
dat<-dat[,-10]
dat<-subset(dat,dat[,4]<0.01 & dat[,6]>0)
...
for(c in 1:length(class)){
	dat2<-rbind(dat2,sub_dat2(subset(dat,Term_type==class[c]),20))
}
...
out<-data.frame(GO_Term=dat2[,1],Name=dat2[,2],Ontology=dat2[,3],Percent=percent[,1],Number=dat2[,6])
...
pdf(paste(args[3],"pdf",sep="."),width=(length(percent[,1])*1.75+15)*0.2,height=25*0.2+3)

par(mar=c(40/2,10,4,5))

barplot(percent[,1],col=as.character(ontos),space=0.75,xaxt="n",yaxt="n",ylim=c(0,tick),main=title)
axis(side=1,at=(0:length(percent[,1]))*1.75+0.375,labels=rep("",length(percent[,1])+1))
#axis(side=2,at=ticks,labels=ticks)
axis(side=2,at=ticks,labels=round(ticks*diff/100))
#mtext(side=2,line=2,"Percent of Genes (%)")
mtext(side=2,line=2,"Number of Genes")
text(1:length(percent[,1])*1.75,-tick/30,labels=labels,xpd=TRUE,srt=60,pos=2)
...
q=o-ln*0.5
s=-0.2-ln*sqrt(3)/2*tick/15
vertile<-0.2+(max(ln)+1)*sqrt(3)/2*tick/15
t=-vertile
p=o-(max(ln)+1)*0.5
tp=-0.2-(max(ln)+1.5)*sqrt(3)/2*tick/15
...
png(paste(args[3],"png",sep="."),width=(length(percent[,1])*1.75+15)*0.2,height=25*0.2+3,units="in",type="cairo-png",res=300)

par(mar=c(40/2,10,4,5))

barplot(percent[,1],space=0.75,col=as.character(ontos),xaxt="n",yaxt="n",ylim=c(0,tick),main=title)
axis(side=1,at=(0:length(percent[,1]))*1.75+0.375,labels=rep("",length(percent[,1])+1))
#axis(side=2,at=ticks,labels=ticks)
axis(side=2,at=ticks,labels=round(ticks*diff/100))
#mtext(side=2,line=2,"Percent of Genes (%)")
