...
my $runcmd = "$perlExec $Bin/pita_run.pl -utr $utr_fn -mir $mir_fn -prefix ${prefix}_ " . 
...
system "rm -rf ${prefix}_ext_utr.stab; mv ${prefix}_pita_results.tab tmp_${pid}; head -n 1 tmp_${pid} | cut -f 1-5,8,10- > ${prefix}_pita_results.tab; cat tmp_${pid} | $perlExec $Bin/body.pl 2 -1 | tr -d '\r' | cut -f 1-8,10- | $perlExec $Bin/merge_columns.pl -1 4 -2 5 -d ':' | $perlExec $Bin/merge_columns.pl -1 4 -2 5 -d ':' | sed 's/Seed:Mismatchs:G:U/Seed/g' | sort -k 13n >> ${prefix}_pita_results.tab; rm tmp_${pid};";

system "cat ${prefix}_pita_results.tab | $perlExec $Bin/body.pl 2 -1 | cut -f 1,2,13 | $perlExec $Bin/modify_column.pl -c 2 -m '\"-1\"' | $perlExec $Bin/average_rows.pl -k 0,1 -losoe -n | $perlExec $Bin/cut.pl -f 2,3,1,4 | $perlExec $Bin/modify_column.pl -c 3 -m '\"-1\"' | $perlExec $Bin/modify_column.pl -c 3 -p 2 | sort -k 4n | $perlExec $Bin/cap.pl \"RefSeq,microRNA,Sites,Score\" > ${prefix}_pita_results_targets.tab";
...
if ($gxp == 1)
{
   ...
   system "$perlExec $Bin/fasta2stab.pl $utr_fn | $perlExec $Bin/stab2length.pl |$perlExec  $Bin/lin.pl | $perlExec $Bin/add_column.pl -s 0 | $perlExec $Bin/cut.pl -f 2,1,4,3 | $perlExec $Bin/add_column.pl -s 0 | $perlExec $Bin/add_column.pl -s 1 | $perlExec $Bin/tab2feature_gxt.pl -n 'UTR Sequences' >> ${prefix}_pita_results.gxp";
   system "$perlExec $Bin/body.pl 2 -1 ${prefix}_pita_results.tab | tr -d '\r' | $perlExec $Bin/cut.pl -f 1-4,2,14 | $perlExec $Bin/uniquify.pl -c 1 | $perlExec $Bin/tab2feature_gxt.pl -n 'PITA Predictions' -c '0,0,255,1'  -lh 50 -l 'Filled box' -minc '0' -maxc '25' >> ${prefix}_pita_results.gxp";
