...
sub GetNumColumns
{
   ...
   my $rows = `head -1 $file_name | $Bin/transpose.pl | wc -l`;
...
sub ExtractFileLinesBetweenRegexps
{
  ...
  if ( $regexp_from eq $regexp_to ) {
    die "($Bin/libfile.pl) ExtractFileLinesBetweenRegexps() - 'from' and 'to' regexps are identical. Give me a break, use grep.\n";
  }
...
sub ExtractAllFileLinesNotBetweenRegexps
{
  ...
  if ( $regexp_from eq $regexp_to ) {
    die "($Bin/libfile.pl) ExtractAllFileLinesNotBetweenRegexps() - 'from' and 'to' regexps are identical. Give me a break, use grep.\n";
  }
