...
while(@ARGV)
{
  ...
  {
    die("$Bin/body.pl: Bad argument '$arg' given.  Use --help for help.");
  }
}
...
{
   $tmp_file = 'tmp_' . time . '.' . rand() . '.$Bin/body.pl';
...
__DATA__
Syntax: $Bin/body.pl BEG END < FILE
...
-f:               Allow giving BEG and END as fractions of number of lines in file (for example $Bin/body.pl 4 -0.2)
