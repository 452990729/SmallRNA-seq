...
{
	dsystem ("cat $utr_fn | $Bin/dos2unix.pl | $Bin/fasta2stab.pl > tmp_utr_stab_$r");
	dsystem ("cat $mir_fn | $Bin/dos2unix.pl | $Bin/fasta2stab.pl > tmp_mir_stab_$r");
}
...
else
{
	...
	{
		dsystem ("cat $upstream_fn | $Bin/fasta2stab.pl > tmp_upstream_stab_$r");
	}
	
	dsystem ("cat tmp_utr_stab_$r " .
			 "| cut -f 1 " .
			 "| $Bin/add_column.pl -s $POLY_A " .
			 "| $Bin/join.pl -o A - tmp_upstream_stab_$r " .
			 "| $Bin/merge_columns.pl -1 1 -2 2 " .
			 "> tmp_ext_utr_$r;");
	
	dsystem ("cat tmp_ext_utr_$r " .
			 "| $Bin/stab2length.pl " .
			 "| $Bin/cut.pl -f 1,1,2,2 " .
			 "| $Bin/modify_column.pl -c 2 -s " . ($CDS_FLANK-1) . " " .
			 ">tmp_ext_utr_pos_$r");
			 
	dsystem ("cat tmp_ext_utr_$r " .
			 "| $Bin/extract_sequence.pl -f tmp_ext_utr_pos_$r " .
			 "> tmp_prefix_$r");
}

my $ext_utr_fn = $output_dir . $prefix . "ext_utr.stab";

dsystem ("$Bin/join.pl tmp_prefix_$r tmp_utr_stab_$r " .
         "| $Bin/add_column.pl -s $POLY_A " .
         "| $Bin/merge_columns.pl -1 1 -2 2 " .
         "| $Bin/merge_columns.pl -1 1 -2 2 " .
         "> " . $ext_utr_fn);
...
dsystem ("$Bin/find_potential_mirna_targets.pl tmp_utr_stab_$r -f tmp_mir_stab_$r $echoed_pt_args > tmp_pt_$r");
...
dsystem ("cat tmp_pt_$r " .
		 "| $Bin/join.pl -1 2 - $ext_utr_fn " .
		 "| $Bin/cut.pl -f 1-9,11- " .
		 "| $Bin/modify_column.pl -c 2,3 -a $CDS_FLANK " .
		 "| $perlExec $Bin/RNAddG_compute.pl -ddgarea $DDG_AREA -upstream_rest $FLANK_UP -downstream_rest $FLANK_DOWN " .
		 "| $Bin/modify_column.pl -c 2,3 -s $CDS_FLANK " .
		 "| $Bin/modify_column.pl -c 9,10,11,12,13,14 -m '\"-1\"' " .
		 "| $Bin/cut.pl -f 1-15,14 " .
		 "| $Bin/modify_column.pl -c 15 -sc 14 " .
		 "| $Bin/cut.pl -f 1-12,14-16,13 " .
		 "| $Bin/modify_column.pl -c 9,10,11,12,13,14,15 -p 2 -m '\"-1\"' " .
		 "| $Bin/cap.pl \"UTR,microRNA,Start,End,Seed,Mismatchs,G:U,Loop,Site size,dGduplex,dG5,dG3,dG0,dG1,dGopen,ddG\" " .
...
__DATA__
syntax: /PUBLIC/source/RNA/smallRNA/pipeline/sRNA/tools_script/PITA/lib/pita_run.pl [OPTIONS]
