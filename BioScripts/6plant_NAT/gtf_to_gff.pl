...
# downloaded from http://seqanswers.com/forums/showthread.php?t=3201&highlight=GFF3
...
#	($g,$tr) = $v[8] =~ /gene_id "(\S+)"; transcript_id "(\S+)"/;
	($g) = $v[8] =~ /gene_id "(\S+)"/;
	($tr) = $v[8] =~ /transcript_id "(\S+)"/;
#	($g,$tr) = $v[8] =~ /gene_id "(ENSGALT.{11})"; transcript_id "(ENSGALT.{11})/;
#print $g,"1\t",$tr,"2\n";
	if ($tr ne $last_tr) {
		push @trs, [@exons];
		if ($g ne $last_g) {
